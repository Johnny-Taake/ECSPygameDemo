"""
Extended resource utilities for the Guess The Number Game.
...
log = get_logger("utils.resources_extended")
...
def load_font_with_fallback(
    ...
    try:
        ...
        log.warning(
            f"Could not load font from {font_path} (size {font_size}), using system font fallback: {e}"
        )
...
def load_image_with_fallback(
    ...
    try:
        ...
        log.warning(f"Could not load image from {image_path}, using placeholder")
        return placeholder
